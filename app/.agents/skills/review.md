...
- Tailwind / Orchid: PascalCase kit props (`variant="Primary"`, `style="Border"`). Button `type` is HTML only. No shadcn `variant="outline"`. `toast.add`, not sonner. No Card. No invented DialogHeader. Lists = SchemaTable only (do not add a second filter/search/pager). Forms = SchemaForm. `CustomerCard` only for customers/beneficiaries. Other people = `ListItem` + `Avatar`.
