...
export const listWakeEvents = createServerFn({ method: 'GET' })
  .validator((data: { key?: string; status?: WakeReceiveStatus; limit?: number } = {}) => data)
...
export const listWakeRows = createServerFn({ method: 'GET' })
  .validator((data: { key?: string; eventId?: string; deliveryStatus?: WakeDeliveryStatus; limit?: number } = {}) => data)
  .handler(async ({ data }): Promise<WakeRow[]> => {
    await requireHitPayRoles(HITPAY_ALL_ROLES)
...
export const updateWakeEventStatus = createServerFn({ method: 'POST' })
  .validator((data: { id: string; status: WakeReceiveStatus; error?: string | null }) => data)
...
export const upsertWakeDestination = createServerFn({ method: 'POST' })
  .validator((data: { key: string; channel: WakeChannel; destination: string; enabled?: boolean }) => data)
  .handler(async ({ data }): Promise<WakeDestination> => {
    await requireHitPayRoles(HITPAY_ALL_ROLES)
