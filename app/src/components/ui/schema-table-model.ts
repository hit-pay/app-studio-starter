...
export type SchemaTableSchema = {
  ...
  sort?: { fields: SchemaTableSortField[]; defaultKey?: string; defaultDir?: 'asc' | 'desc' } | false
...
export function defaultSchemaTableQuery(schema: SchemaTableSchema): SchemaTableQuery {
  const pageSize =
    schema.pagination === false ? Number.POSITIVE_INFINITY : (schema.pagination?.pageSize ?? 10)
...
export function searchKeys(schema: SchemaTableSchema) {
  const fromColumns = schema.columns.filter((column) => column.search !== false).map((column) => column.key)
  return fromColumns.length > 0 ? fromColumns : schema.columns.map((column) => column.key)
}
...
export function matchesSearch(row: SchemaTableRow, query: string, keys: string[]) {
  const needle = query.trim().toLowerCase()
  if (!needle) return true
  return keys.some((key) => cellString(row[key]).toLowerCase().includes(needle))
}

export function matchesTab(row: SchemaTableRow, schema: SchemaTableSchema, tabKey: string) {
...
export function matchesFilters(row: SchemaTableRow, filters: Record<string, string>) {
...
export function compareRows(
  ...
  else result = cellString(a).localeCompare(cellString(b), undefined, { numeric: true })
  return sortDir === 'asc' ? result : -result
}

export function defaultColumnOrder(schema: SchemaTableSchema) {
  return schema.columns.filter((column) => !column.locked).map((column) => column.key)
}

export function defaultHiddenKeys(schema: SchemaTableSchema) {
  return schema.columns.filter((column) => column.hidden && !column.locked).map((column) => column.key)
}
...
export function orderedVisibleColumns(
  ...
  const active = columnOrder
    ...
    .filter((column): column is SchemaTableColumn =>
...
export function queryTable({
  ...
  const sorted =
    schema.mode === 'server' || !query.sortKey
      ? filtered
      : [...filtered].sort((left, right) => compareRows(left, right, query.sortKey!, query.sortDir))

  const filteredCount = schema.mode === 'server' ? (total ?? data.length) : sorted.length
  const pageSize = query.pageSize === Number.POSITIVE_INFINITY ? sorted.length || 1 : query.pageSize
  const pageCount = Math.max(1, Math.ceil(filteredCount / pageSize) || 1)
  const page = Math.min(query.page, pageCount)
  const start = (page - 1) * pageSize
  const rows = schema.mode === 'server' ? data : sorted.slice(start, start + pageSize)
...
export const SCHEMA_TABLE_EXAMPLE_ROWS: SchemaTableRow[] = Array.from({ length: 36 }, (_, index) => {
  ...
  return {
    ...
    category: index % 5 === 0 ? 'Apparel' : index % 5 === 1 ? 'Membership' : 'Workshop',
