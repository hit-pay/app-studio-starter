...
function DataListMore({ menu }: { menu: ReactNode }) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        nativeButton
        className="inline-flex size-7 cursor-pointer items-center justify-center rounded-lg p-0.5 opacity-0 outline-none group-hover/data-list-item:opacity-100"
        render={
      ...
      {actions.map((action, index) => (
        <span key={action.key} className="inline-flex items-center">
          ...
          <button
            ...
            className={cn(
              'inline-flex size-6 cursor-pointer items-center justify-center rounded p-1 outline-none',
