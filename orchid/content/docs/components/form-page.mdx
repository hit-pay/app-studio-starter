import { FormPageDemo } from '../../../src/components/doc/demos/form-page-demo'
...
`FormPageHeader` uses **Cancel** and **Save** by default. Pass `formId` at the top level to connect Save to a form, and provide only the action properties that need customization. Cancel uses `onClose` unless `actions.cancel.onClick` is supplied.
