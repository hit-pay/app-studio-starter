...
UI primitives install under `src/components/ui` and import from
`@ui/<name>`. Blocks install under `src/components/{category}`
...
Use base primitives from `@ui/<name>` only when no Orchid block covers the
...
are installed automatically. Import a UI primitive from `@ui/button`
