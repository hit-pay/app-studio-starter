<!-- Generated from content/docs/components/drawer.mdx. Do not edit. -->
...
Use `Drawer` from `@/base-ui/overlays/drawer`. Set `swipeDirection` to `up`, `right`, `down`, or `left`. `DrawerContent` composes portal, overlay, viewport, and popup. Pass `showSwipeHandle` when the drawer should show a drag affordance. Do not use Sheet.
