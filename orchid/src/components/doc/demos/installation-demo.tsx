...
const AGENTS = [
  {
    name: "Cursor",
    instruction:
      "Open an empty parent folder, start an Agent chat, and paste the prompt below.",
  },
  {
    name: "Claude Code",
    instruction:
      "Start Claude Code where the new project should be created, then paste the prompt.",
  },
  {
    name: "Other coding agents",
    instruction:
      "Use any local agent that can create files and run terminal commands.",
  },
] as const;
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-solid border-oc-border bg-oc-muted">
      <div className="flex items-center justify-between gap-3 border-b border-solid border-oc-border px-3 py-2">
        <span className="font-mono text-xs text-oc-muted-foreground">
          Installation prompt
        </span>
  ...
  return (
    <>
      <header className="flex min-w-0 flex-col gap-1">
        <h1 className="wrap-break-word text-lg leading-6 font-medium text-oc-foreground">
          Installation
        </h1>
        <p className="wrap-break-word text-sm leading-5 text-oc-muted-foreground">
          Create a new Orchid UI app with Cursor, Claude Code, or another local
          coding agent.
        </p>
      </header>
      ...
      <Section title="1. Your agent asks you to choose a framework">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {FRAMEWORKS.map((framework) => (
            ...
            >
              <h3 className="text-sm font-medium text-oc-card-foreground">
                {framework.name}
              </h3>
      ...
      <Section title="3. Paste this prompt">
        <p className="max-w-3xl text-sm leading-6 text-oc-muted-foreground">
          Replace the project name and app request placeholders. The agent will
          ask which framework you prefer before it creates files, then configure
          Orchid and verify the build.
        </p>
