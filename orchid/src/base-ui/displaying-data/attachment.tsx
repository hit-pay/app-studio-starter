...
const attachmentMediaVariants = cva(
  "relative flex aspect-square w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-oc-muted text-oc-foreground group-data-[orientation=vertical]/attachment:w-full group-data-[size=sm]/attachment:w-8 group-data-[size=xs]/attachment:w-7 group-data-[size=xs]/attachment:rounded-md group-data-[state=error]/attachment:bg-oc-destructive/10 group-data-[state=error]/attachment:text-oc-destructive group-data-[orientation=vertical]/attachment:*:data-[slot=spinner]:size-6! [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 group-data-[orientation=vertical]/attachment:[&_svg:not([class*='size-'])]:size-6 group-data-[size=xs]/attachment:[&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        icon: '',
...
function AttachmentDescription({ className, ...props }: ComponentProps<'span'>) {
  return (
    <span
      ...
      className={cn(
        'mt-0.5 block max-w-full min-w-0 truncate text-xs text-oc-muted-foreground group-data-[state=error]/attachment:text-oc-destructive/80',
        className,
      )}
      {...props}
    />
